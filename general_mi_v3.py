...
class MIArray(np.ndarray):  
  ...
  def load_data(self, item):
    ...
    if self.nick_name == RAW:
      ...
      raise ValueError(f'Unknown nick_name {self.nick_name}')
    return val
...
class AbstractGeneralMI:
  def __init__(self, images_dict, image_keys=None, label_keys=None,
    ...
    self.raw_process = (lambda x: x) if self.raw_process is None else self.raw_process
    self.data_process = (lambda x: x) if self.data_process is None else self.data_process
    self.post_process = (lambda x: x) if self.post_process is None else self.post_process
  ...
  def __getitem__(self, item):
    ...
    for key in image_dict.keys():
      ...
      if key in self.image_keys:
        image_dict[key][RAW] = self.images_dict[key][RAW][item]
  ...
  def image_keys(self, value):
    ...
    for key in value:
      ...
      if key not in self.image_keys:
        ...
        length = len(self.images_dict[key][PATH])
        self.images_dict[key][PATH] = self.images_dict[key][PATH].view(MIArray)
...
class GeneralMI(AbstractGeneralMI):
  ...
  def get_test_sample(cls, csv_path):
    ...
    test = cls(img_dict, ['30G', '60G-3', '60G-2', 'CT', '240G', '240S'], pid,
               img_type=img_type)
    test.rm_void_data()
    return test
